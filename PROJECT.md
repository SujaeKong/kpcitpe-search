...
- **정보관리 4교시만 통합본 폴백**: 4교시는 "6문제 중 4문제 선택" 형식이고 해설집이 문제 1·2·4·6만 다룸 → 검출 4 ≠ 문항 6으로 가드 작동(정상). 3·5번은 해설 자체가 없음.
- **자동 split은 `SPLIT_MODE: test`(고정 샘플 31 task)** 라 신규 회차가 포함되지 않음 → 신규 회차는 `Split PDFs` 워크플로를 `mode=all` + `only`로 수동 실행해야 함.
