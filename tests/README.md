...
> 캡처 시(2026-09-15) 제목 정렬 가드가 기출 87회 1교시 정보관리(기존 재분할 금지)와 **컴시응(운영 중 분할본)** 을 어긋남으로 판정 → 컴시응 분할본을 Drive에서 열어 보니 `04_WiBro.pdf` 안이 PDF 3번 Haptics, `14_Haptics.pdf` 안이 13번 RFID로 4~14번이 실제로 어긋나 있어 통합본 폴백·재분할 금지 추가. 이미 분할된 전체 항목은 `split-audit` 워크플로로 같은 가드를 다시 검사한다.
